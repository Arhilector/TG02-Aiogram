...
async def start_command(message: Message):
    await message.answer(
        f'Привет, {message.from_user.full_name}! Я медиа-бот. Выберите действие:',
...
async def process_voice_callback(callback: types.CallbackQuery):
    await callback.answer()
    await send_voice(callback.message)
...
async def send_voice(message: Message):
    try:
        text = "Это пример голосового сообщения от бота."
        tts = gTTS(text=text, lang='ru')
        file_path = os.path.join('img', 'voice_message.ogg')
        tts.save(file_path)
        voice = FSInputFile(file_path)
        await message.answer_voice(voice=voice, caption="Вот ваше голосовое сообщение")
    except Exception as e:
        print(f"Ошибка при отправке голосового сообщения: {e}")
        await message.answer("Извините, произошла ошибка при отправке голосового сообщения.")
...
async def handle_voice(message: Message):
    try:
        ...
        file_path = os.path.join('img', f'{file.file_id}.ogg')
        await bot.download_file(file.file_path, file_path)
        await message.answer("Ваша аудиозапись сохранена в базу данных.")
    except Exception as e:
